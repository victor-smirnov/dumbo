...
#include <dumbo/v1/fiber/detail/cpu_relax.hpp>
#include <dumbo/v1/fiber/detail/futex.hpp>
...
public:
    ...
    void lock() noexcept {
        ...
        const std::int32_t max_tests = (std::min)( static_cast< std::int32_t >( DUMBO_FIBERS_SPIN_MAX_TESTS), 2 * prev_tests + 10);
        // after max. spins or collisions suspend via futex
        while ( max_tests > tests && DUMBO_FIBERS_SPIN_MAX_COLLISIONS > collisions) {
            ...
            if ( 0 != ( expected = value_.load( std::memory_order_relaxed) ) ) {
                ++tests;
#if !defined(DUMBO_FIBERS_SPIN_SINGLE_CORE)
