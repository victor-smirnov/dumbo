...
struct condition_test_data {
    ...
    dumbo::v1::fibers::condition_variable cond;
    int notified;
    int awoken;
};

void condition_test_fiber(condition_test_data* data) {
    std::unique_lock<dumbo::v1::fibers::mutex> lock(data->mutex);
...
void notify_one_fn( dumbo::v1::fibers::condition_variable & cond) {
	cond.notify_one();
}

void notify_all_fn( dumbo::v1::fibers::condition_variable & cond) {
	cond.notify_all();
}
...
void wait_fn(
	...
	dumbo::v1::fibers::condition_variable & cond) {
	std::unique_lock< dumbo::v1::fibers::mutex > lk( mtx);
	cond.wait( lk);
	++value;
}
...
void test_one_waiter_notify_one() {
    ...
    dumbo::v1::fibers::fiber f1(
                dumbo::v1::fibers::launch::dispatch,
                wait_fn,
                std::ref( mtx),
                std::ref( cond) );
...
void fn1( dumbo::v1::fibers::mutex & m, dumbo::v1::fibers::condition_variable & cv) {
    std::unique_lock< dumbo::v1::fibers::mutex > lk( m);
    BOOST_CHECK(test2 == 0);
    test1 = 1;
    cv.notify_one();
    while (test2 == 0) {
...
void fn2( dumbo::v1::fibers::mutex & m, dumbo::v1::fibers::condition_variable & cv) {
    std::unique_lock< dumbo::v1::fibers::mutex > lk( m);
    BOOST_CHECK(test2 == 0);
    test1 = 1;
    cv.notify_one();
    ...
    int count=0;
    while (test2 == 0 && cv.wait_until(lk, t) == dumbo::v1::fibers::cv_status::no_timeout)
        count++;
...
void fn3( dumbo::v1::fibers::mutex & m, dumbo::v1::fibers::condition_variable & cv) {
    std::unique_lock< dumbo::v1::fibers::mutex > lk( m);
    BOOST_CHECK(test2 == 0);
    test1 = 1;
    cv.notify_one();
...
void fn4( dumbo::v1::fibers::mutex & m, dumbo::v1::fibers::condition_variable & cv) {
    std::unique_lock< dumbo::v1::fibers::mutex > lk( m);
    BOOST_CHECK(test2 == 0);
    test1 = 1;
    cv.notify_one();
    std::chrono::steady_clock::time_point t0 = std::chrono::steady_clock::now();
    int count=0;
    while (test2 == 0 && cv.wait_for(lk, ms(250)) == dumbo::v1::fibers::cv_status::no_timeout)
        count++;
    std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
    if (runs == 0) {
        BOOST_CHECK(t1 - t0 < ms(250));
...
void fn5( dumbo::v1::fibers::mutex & m, dumbo::v1::fibers::condition_variable & cv) {
    std::unique_lock< dumbo::v1::fibers::mutex > lk( m);
    BOOST_CHECK(test2 == 0);
    test1 = 1;
    cv.notify_one();
...
void do_test_condition_wait() {
    ...
    dumbo::v1::fibers::condition_variable cv;
    std::unique_lock< dumbo::v1::fibers::mutex > lk( m);
    dumbo::v1::fibers::fiber f( dumbo::v1::fibers::launch::dispatch, & fn1, std::ref( m), std::ref( cv) );
    BOOST_CHECK(test1 == 0);
    while (test1 == 0)
        cv.wait(lk);
    BOOST_CHECK(test1 != 0);
...
void test_condition_wait() {
    dumbo::v1::fibers::fiber( dumbo::v1::fibers::launch::dispatch, & do_test_condition_wait).join();
...
void do_test_condition_wait_until() {
    ...
    {
        std::unique_lock< dumbo::v1::fibers::mutex > lk( m);
        dumbo::v1::fibers::fiber f( dumbo::v1::fibers::launch::dispatch, & fn2, std::ref( m), std::ref( cv) );
        BOOST_CHECK(test1 == 0);
        while (test1 == 0)
            cv.wait(lk);
        BOOST_CHECK(test1 != 0);
...
void test_condition_wait_until() {
    dumbo::v1::fibers::fiber( dumbo::v1::fibers::launch::dispatch, & do_test_condition_wait_until).join();
...
void do_test_condition_wait_until_pred() {
    ...
    {
        std::unique_lock< dumbo::v1::fibers::mutex > lk( m);
        dumbo::v1::fibers::fiber f( dumbo::v1::fibers::launch::dispatch, & fn3, std::ref( m), std::ref( cv) );
        BOOST_CHECK(test1 == 0);
        while (test1 == 0)
            cv.wait(lk);
        BOOST_CHECK(test1 != 0);
...
void test_condition_wait_until_pred() {
    dumbo::v1::fibers::fiber( dumbo::v1::fibers::launch::dispatch, & do_test_condition_wait_until_pred).join();
...
void do_test_condition_wait_for() {
    ...
    {
        std::unique_lock< dumbo::v1::fibers::mutex > lk( m);
        dumbo::v1::fibers::fiber f( dumbo::v1::fibers::launch::dispatch, & fn4, std::ref( m), std::ref( cv) );
        BOOST_CHECK(test1 == 0);
        while (test1 == 0)
            cv.wait(lk);
        BOOST_CHECK(test1 != 0);
...
void test_condition_wait_for() {
    dumbo::v1::fibers::fiber( dumbo::v1::fibers::launch::dispatch, & do_test_condition_wait_for).join();
...
void do_test_condition_wait_for_pred() {
    ...
    {
        std::unique_lock< dumbo::v1::fibers::mutex > lk( m);
        dumbo::v1::fibers::fiber f( dumbo::v1::fibers::launch::dispatch, & fn5, std::ref( m), std::ref( cv) );
        BOOST_CHECK(test1 == 0);
        while (test1 == 0)
            cv.wait(lk);
        BOOST_CHECK(test1 != 0);
...
void test_condition_wait_for_pred() {
    dumbo::v1::fibers::fiber( dumbo::v1::fibers::launch::dispatch, & do_test_condition_wait_for_pred).join();
