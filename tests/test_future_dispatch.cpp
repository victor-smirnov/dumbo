...
void test_future_create_void() {
    // default constructed future is not valid
    dumbo::v1::fibers::future< void > f1;
    BOOST_CHECK( ! f1.valid() );

    // future retrieved from promise is valid (if it is the first)
    dumbo::v1::fibers::promise< void > p2;
    dumbo::v1::fibers::future< void > f2 = p2.get_future();
    BOOST_CHECK( f2.valid() );
}
...
void test_future_move() {
    ...
    // move construction
    dumbo::v1::fibers::future< int > f2( std::move( f1) );
    BOOST_CHECK( ! f1.valid() );
    BOOST_CHECK( f2.valid() );
...
void test_future_move_void() {
    ...
    // move construction
    dumbo::v1::fibers::future< void > f2( std::move( f1) );
    BOOST_CHECK( ! f1.valid() );
    BOOST_CHECK( f2.valid() );
...
void test_future_get_move() {
    ...
    dumbo::v1::fibers::future< A > f1 = p1.get_future();
    BOOST_CHECK( f1.valid() );
